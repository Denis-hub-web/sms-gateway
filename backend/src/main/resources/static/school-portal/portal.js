...
// ── Session Management (strict: sessionStorage, 8h inactivity) ──
const SESSION_TIMEOUT_MS = 8 * 60 * 60 * 1000; // 8 hours
let _inactivityTimer = null;

function getToken()    { return sessionStorage.getItem('schoolToken'); }
function getTenant()   { return JSON.parse(sessionStorage.getItem('schoolTenant') || 'null'); }
function saveSession(token, tenant) {
    sessionStorage.setItem('schoolToken', token);
    sessionStorage.setItem('schoolTenant', JSON.stringify(tenant || {}));
    sessionStorage.setItem('schoolSessionStart', Date.now().toString());
    resetInactivityTimer();
}
function clearSession() {
    sessionStorage.removeItem('schoolToken');
    sessionStorage.removeItem('schoolTenant');
    sessionStorage.removeItem('schoolSessionStart');
...
function resetInactivityTimer() {
    ...
    _inactivityTimer = setTimeout(() => {
        clearSession();
        sessionStorage.setItem('schoolLoginMsg', 'Session expired due to inactivity. Please sign in again.');
...
// Reset inactivity on any user activity
...
async function handleLogin() {
    ...
    btn.disabled = true;
    btn.innerHTML = '<i data-lucide="loader" class="spin-icon"></i> Signing in...';
    if (window.lucide) lucide.createIcons();
    ...
    const msg = sessionStorage.getItem('schoolLoginMsg');
    if (msg) {
        showAlert('alertError', `<i data-lucide="alert-circle" class="alert-icon"></i> ${msg}`, 'danger');
        sessionStorage.removeItem('schoolLoginMsg');
        if (window.lucide) lucide.createIcons();
    }
    ...
    try {
        ...
        // Fetch tenant info to check subscription/approval status
        const meRes = await fetch(`${PORTAL_API}/api/school/me`, {
            headers: { 'Authorization': `Bearer ${data.accessToken}` }
        });

        if (meRes.status === 401 || meRes.status === 403) throw new Error('Unauthorized — contact admin.');

        const tenant = meRes.ok ? await meRes.json() : {};
        ...
        // Strict session — use sessionStorage only
        saveSession(data.accessToken, tenant);
...
async function initDashboard() {
    ...
    try {
        ...
        saveSession(getToken(), tenant); // keep session fresh & use sessionStorage
...
function autoLinkApiKey() {
    ...
    const key = inp.value;
    sessionStorage.setItem('cfg_api_key', key);
...
function doLogout() {
    clearSession();
    sessionStorage.setItem('schoolLoginMsg', 'You have been signed out.');
    window.location.href = 'login.html';
}
